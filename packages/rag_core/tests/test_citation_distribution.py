...
def _payload(*, domain_id: str, request_id: str, citation_types: list[str]) -> ChatLogPayload:
...
async def test_distribution_aggregates_tenant_records_in_all_bucket():
    ...
    await writer.write(_payload(domain_id="t1", request_id="r1", citation_types=["direct"]))
    await writer.write(
        _payload(domain_id="t1", request_id="r2", citation_types=["direct", "synthesis"])
    )
    await writer.write(
        _payload(domain_id="t1", request_id="r3", citation_types=["inference"])
    )
    # 다른 tenant — 격리
    await writer.write(_payload(domain_id="t2", request_id="r4", citation_types=["direct"]))

    analytics = InMemoryCitationDistributionAnalytics(writer=writer)
    result = await analytics.distribution(
        domain_id="t1", from_date=None, to_date=None, group_by="day",
    )
...
async def test_distribution_empty_tenant_returns_no_buckets():
    ...
    analytics = InMemoryCitationDistributionAnalytics(writer=writer)
    result = await analytics.distribution(
        domain_id="ghost", from_date=None, to_date=None, group_by="hour",
    )
