...
def _doc(domain_id: str, doc_id: str, version: str = "v1") -> DocumentRecord:
    return DocumentRecord(domain_id=domain_id, doc_id=doc_id, title=doc_id, version=version)


def _chunk(domain_id: str, doc_id: str, idx: int, *, archived: bool = False) -> ChunkRecord:
    rec = ChunkRecord(
        domain_id=domain_id, chunk_id=f"{doc_id}:c{idx}", doc_id=doc_id,
...
def _payload(
    *,
    ...
    return ChatLogPayload(
        domain_id=domain_id, request_id=request_id, user_id="u1",
...
async def analytics():
    ...
    await chunks.replace_chunks(
        domain_id="t1", doc_id="DOC-1", doc_version="v1",
        parser_version="p1",
        chunks=[_chunk("t1", "DOC-1", 0), _chunk("t1", "DOC-1", 1, archived=True),
                _chunk("t1", "DOC-1", 2)],
    )
    await chunks.replace_chunks(
        domain_id="t1", doc_id="DOC-2", doc_version="v1",
        parser_version="p1",
        chunks=[_chunk("t1", "DOC-2", 0), _chunk("t1", "DOC-2", 1)],
    )
    # 다른 tenant chunks
    await chunks.replace_chunks(
        domain_id="t2", doc_id="DOC-X", doc_version="v1",
        parser_version="p1",
    ...
    await jobs.create(IndexingJobRecord(
        job_id="J-1", domain_id="t1", doc_id="DOC-1", doc_version="v1", status="completed"
    ))
    await jobs.create(IndexingJobRecord(
        job_id="J-2", domain_id="t1", doc_id="DOC-2", doc_version="v1", status="completed"
    ))
    await jobs.create(IndexingJobRecord(
        job_id="J-3", domain_id="t1", doc_id="DOC-3", doc_version="v1", status="failed"
    ))
    # 다른 tenant
    await jobs.create(IndexingJobRecord(
        job_id="J-X", domain_id="t2", doc_id="DOC-X", doc_version="v1", status="completed"
    ))

    # chat_logs
    await writer.write(_payload(
        domain_id="t1", request_id="r1", citation_types=["direct"], latency_ms=100,
    ))
    await writer.write(_payload(
        domain_id="t1", request_id="r2", citation_types=["direct", "synthesis"],
        latency_ms=200,
    ))
    # 인용 없는 답변
    await writer.write(_payload(
        domain_id="t1", request_id="r3", citation_types=[], citations=[],
        latency_ms=300,
    ))
    # fallback (citations 빈 — answers_without_citation 누적)
    await writer.write(_payload(
        domain_id="t1", request_id="r4", citation_types=[], citations=[],
        fallback_reason="low_retrieval", latency_ms=50,
    ))
    await writer.write(_payload(
        domain_id="t1", request_id="r5", citation_types=[], citations=[],
        fallback_reason="low_retrieval", latency_ms=80,
    ))
    # shared_llm routing
    await writer.write(_payload(
        domain_id="t1", request_id="r6", citation_types=["inference"],
        routing_decision={"selected_model": "shared_llm", "selected_lora": None},
        latency_ms=400,
    ))
    # 다른 tenant
    await writer.write(_payload(domain_id="t2", request_id="rX"))
...
async def test_total_documents_tenant_scope(analytics):
    snap = await analytics.get_snapshot(domain_id="t1")
    assert snap.total_documents == 3
    # t2의 DOC-X는 미포함
    snap_t2 = await analytics.get_snapshot(domain_id="t2")
...
async def test_empty_tenant_returns_zeros():
    ...
    snap = await analytics.get_snapshot(domain_id="ghost")
