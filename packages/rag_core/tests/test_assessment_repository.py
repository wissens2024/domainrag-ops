...
def _item(item_id: str, **kwargs) -> AssessmentItemRecord:
    base = dict(
        item_id=item_id, domain_id=kwargs.pop("domain_id", "t1"),
...
async def test_upsert_and_get():
    ...
    got = await repo.get(domain_id="t1", item_id="Q-1")
...
async def test_list_by_tenant_keyword_and_filters():
    ...
    items, total = await repo.list_by_tenant(
        domain_id="t1", keyword="패스워드",
    )
    assert total == 1
    assert items[0].item_id == "Q-1"

    items, total = await repo.list_by_tenant(
        domain_id="t1", subject="네트워크",
    )
    assert total == 1
...
async def test_touch_used_increments_and_sets_last_used():
    ...
    affected = await repo.touch_used(domain_id="t1", item_ids=["Q-1", "Q-2", "ghost"])
    assert affected == 2
    rec = await repo.get(domain_id="t1", item_id="Q-1")
...
async def test_review_queue_returns_draft_and_reviewed():
    ...
    await repo.upsert(_item("Q-A", quality_status="approved"))
    items, total = await repo.list_review_queue(domain_id="t1")
...
async def test_tenant_isolation():
    repo = InMemoryAssessmentItemRepository()
    await repo.upsert(_item("Q-1", domain_id="t1"))
    await repo.upsert(_item("Q-1", domain_id="t2"))  # 다른 tenant 같은 item_id
    t1_item = await repo.get(domain_id="t1", item_id="Q-1")
    t2_item = await repo.get(domain_id="t2", item_id="Q-1")
    assert t1_item is not None and t2_item is not None
    assert t1_item.domain_id == "t1"
    assert t2_item.domain_id == "t2"
