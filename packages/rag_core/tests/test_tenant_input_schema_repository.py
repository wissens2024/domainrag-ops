...
async def test_insert_initial_active_version_1():
    ...
    rec = await repo.insert_new_active(
        tenant_id="t1", base_version=None,
...
async def test_get_active_returns_latest_active():
    ...
    record = await repo.get_active(tenant_id="t1")
...
async def test_insert_with_correct_base_increments_and_deprecates():
    ...
    history = await repo.list_history(tenant_id="t1")
...
async def test_list_history_pagination():
    ...
    await repo.insert_new_active(tenant_id="t1", base_version=None,
                                  schema_yaml=_BASE_YAML, ui_schema_yaml={})
    await repo.insert_new_active(tenant_id="t1", base_version=1,
                                  schema_yaml=_BASE_YAML, ui_schema_yaml={})
    await repo.insert_new_active(tenant_id="t1", base_version=2,
                                  schema_yaml=_BASE_YAML, ui_schema_yaml={})

    page1 = await repo.list_history(tenant_id="t1", limit=2, offset=0)
    assert [r.schema_version for r in page1] == [3, 2]
    page2 = await repo.list_history(tenant_id="t1", limit=2, offset=2)
...
async def test_tenant_isolation():
    ...
    await repo.insert_new_active(tenant_id="t1", base_version=None,
                                  schema_yaml=_BASE_YAML, ui_schema_yaml={})
    await repo.insert_new_active(tenant_id="t2", base_version=None,
                                  schema_yaml=_BASE_YAML, ui_schema_yaml={})
    # 두 tenant 모두 version=1로 시작 (전역 UNIQUE가 아니라 (tenant, version))
    t1 = await repo.get_active(tenant_id="t1")
    t2 = await repo.get_active(tenant_id="t2")
