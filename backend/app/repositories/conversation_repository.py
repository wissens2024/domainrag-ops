...
흐름:
  ...
  2. SQL: SELECT/UPDATE/DELETE — domain_id 필터는 RLS가 자동, 추가로 user_id
                ...
                await session.execute(
                    text(
                        ...
                        SELECT c.id, c.title, c.created_at, c.updated_at,
                               (SELECT COUNT(*)
                                  FROM chat_logs cl
                                 WHERE cl.domain_id = c.domain_id
                                   AND cl.conversation_id = c.id) AS msg_count
                          FROM conversations c
                         WHERE c.domain_id = :domain_id AND c.user_id = :user_id
    ...
    async def update_title(
        ...
        *,
        ...
        return await self.get(
            domain_id=domain_id, user_id=user_id, conversation_id=conversation_id
        )
