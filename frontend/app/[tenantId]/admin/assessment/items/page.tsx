...
export default function ItemBankPage() {
  ...
  const swrKey = `assessment:${tenantId}:${JSON.stringify(filters)}:${page}`;
  const { data, isLoading } = useSWR<AssessmentListResult>(
    tenantId ? swrKey : null,
    () =>
      listAssessmentItems(tenantId, {
  ...
  const handleApprove = async (itemId: string) => {
    ...
    try {
      await approveAssessmentItem(tenantId, itemId);
      void mutate(swrKey);
    } catch (e) {
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2 text-sm">
          <Link
            href={`/${tenantId}/admin/assessment/workbench`}
            className="px-3 py-1 border rounded"
          >
            Workbench
          </Link>
          <Link
            href={`/${tenantId}/admin/assessment/review-queue`}
            className="px-3 py-1 border rounded"
          >
            Review Queue
          </Link>
