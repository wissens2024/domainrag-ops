...
export default function DocumentUploadPage() {
  ...
  const { data, error: schemaError } = useSWR<{ items: SchemaListItem[] }>(
    tenantId ? `input_schemas:${tenantId}` : null,
    () => listInputSchemas(tenantId),
  );
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const result = await uploadDocument(tenantId, file, metadata, selectedType);
      alert(
        `업로드 완료\njob_id: ${result.job_id}\ndoc_id: ${result.doc_id}\n인덱싱 모니터링에서 진행 상황 확인 가능.`,
      );
      router.push(`/${tenantId}/admin/indexing`);
    } catch (e) {
