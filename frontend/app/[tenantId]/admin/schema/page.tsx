...
export default function SchemaEditorPage() {
  ...
  const { data: current, mutate: refresh } = useSWR<InputSchemaRecord>(
    ...
    () => getSchema(tenantId).catch(() => null as unknown as InputSchemaRecord),
  );

  const { data: history } = useSWR<InputSchemaHistory>(
    tenantId && showHistory ? `schema-history:${tenantId}` : null,
    () => getSchemaHistory(tenantId, { page_size: 20 }),
  );
  ...
  const handleSave = async () => {
    ...
    try {
      const result = await putSchema(tenantId, {
        schema_yaml: parsed,
