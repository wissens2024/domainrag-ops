...
export default function TenantConfigsPage() {
  ...
  const swrKey = `configs:${domainId}:${category}`;
  const { data, isLoading } = useSWR<Record<string, unknown>>(
    domainId ? swrKey : null,
    () => getConfigCategory(domainId, category),
  );

  const historyKey = `configs-history:${domainId}:${category}`;
  const { data: history } = useSWR(
    domainId ? historyKey : null,
    () => getConfigHistory(domainId, { category, page_size: 10 }),
  );
  ...
  const handlePatch = async () => {
    ...
    try {
      await patchConfig(domainId, category, {
        key: pathInput,
        value: parsed,
        reason,
      });
