...
export default function DocumentDetailPage() {
  const params = useParams<{ domainId: string; docId: string }>();
  ...
  const { data, error, isLoading, mutate } = useSWR<DocumentDetail>(
    domainId && docId ? `doc:${domainId}:${docId}:${version}` : null,
    () => getDocument(domainId, docId, version),
  );
  ...
  const handleEditMetadata = async () => {
    ...
    try {
      await patchDocumentMetadata(domainId, docId, {
  ...
  const handleHardDelete = async () => {
    ...
    try {
      const result = await hardDeleteDocument(domainId, docId, {
        reason,
      ...
      router.push(`/${domainId}/admin/documents`);
    } catch (e) {
